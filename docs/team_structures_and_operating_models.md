...
**Harness mapping:** the pod *is* the existing [Pod](pods_and_skill_routing.md). `team-alpha` and `team-beta` are simply two pods/teams; the harness lets you configure **any** team's shape — they are no longer hardcoded routers.

```yaml
team:
  name: team-alpha            # a configured team, not a built-in
...
```yaml
team:
  name: team-alpha                 # any team — team-alpha/team-beta are just examples
