...
| **4. Sandbox & Compute** | AgentCube / Kata references in Team-alpha skills | `agentic-harness` (pod-bundle skills) | **Missing** (docs only) | Kubernetes skill content describes AgentCube serverless sandboxes and Kata isolation—reference material, not shipped runtime. | No integrated compute layer in any air repo. |
