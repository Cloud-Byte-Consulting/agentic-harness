...
- **Tier 1 — ownership routing:** the task's domain picks a router (e.g. `team-alpha-router` for Kubernetes, `team-beta-router` for APIM/.NET).
...
- **Domain router-skills** — [`pod-bundle/router-skills/`](../pod-bundle/router-skills/): **Team-alpha** (14 Kubernetes/Volcano skills) and **TEAM-BETA** (5 Azure APIM/.NET skills), each staged for promotion via a `PR-README.md`.
...
The thumb-drive bundle is the **pattern library**; [`Cachy/pods/cachy-core-platform/`](../../Cachy/pods/cachy-core-platform/) is a **concrete instantiation**. Cachy kept the five-file contract and the shared disciplines (mutation gate, evidence-grounded investigation, role separation) but defined its *own* routers (`cachy-core`, `cachy-desktop`, `cachy-extensions`, `cachy-ops`) and dropped the Team-alpha/TEAM-BETA domains it doesn't use. The porting analysis is recorded in [`Cachy/docs/pod-bundle-analysis.md`](../../Cachy/docs/pod-bundle-analysis.md).
