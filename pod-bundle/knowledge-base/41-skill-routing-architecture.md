...
flowchart TD
    ...
    R -->|TEAM-BETA| TR[team-beta-router skill set<br/>APIM · Azure · ASP.NET]
    R -->|Team-alpha| CR[team-alpha-router skill set<br/>K8s · Grafana · GitHub]
...
1. **Tier-1 — ownership scoping (brain).** `/platform-router` / `scripts/co-dispatch.ps1`
   ...
   scope. A UI/TypeScript task gets `clean-code-typescript` (shared), never Team-alpha k8s.
2. **Tier-2 — skill auto-trigger (progressive disclosure).** Within the in-scope set, each
   ...
   body loads on trigger, `references/` only on deeper demand. A Team-alpha *networking*
...
| --- | --- | --- |
| Team-alpha (11 k8s) | `router-skills/team-alpha/` | PR into `../team-alpha-agent-system/.claude/skills` |
| TEAM-BETA (5: APIM/CSPM/.NET) | `router-skills/team-beta/` | PR into `../team-beta-agent-system/.claude/skills` |
...
TEAM-BETA/Team-alpha repos are **production** — their skills stage here and promote via a
...
is a production hotfix (Grande / TEAM-BETA prod).
...
| Bonete pod won't schedule | team-alpha | `kubernetes-autoscaling-scheduling` | `kubernetes-storage`, TEAM-BETA skills |
| APIM rate-limit policy review | team-beta | `api-management` | all k8s, .NET fundamentals |
| TEAM-BETA snapshot API 500s | team-beta | `dotnet-web-development` | all k8s, APIM |
| ICM: Grande latency spike | team-alpha | `kubernetes-observability`, `security-operations-mitre-attack` | `-storage`, `-gitops-cicd` |
